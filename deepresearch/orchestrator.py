...
def decompose_query(query: str, intent: str) -> List[str]:
    """Decompose a research query into sub-questions. Uses cache if available."""
    # Check cache first
    from .cache import get_cached_sub_questions, cache_sub_questions

    cached = get_cached_sub_questions(query, intent)
    if cached:
        return cached
    ...
    sub_qs = sub_qs[:5]  # Cap at 5
    cache_sub_questions(query, intent, sub_qs)
    return sub_qs
